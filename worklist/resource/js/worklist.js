...
$(document).ready(function () {
	let worklist = (function () {
		...
		let fileDir = Util.getDirName();
		let url_info = String(window.location.href.slice(window.location.href.indexOf('?') + 1).split('&'));
		let activeTabNum = rMap.tab_navList.size() - 2;
		let guideTabNum = rMap.tab_navList.size() - 1; //가이드 링크
		let allTabNum = rMap.tab_navList.size() - 1; //전체 보기
		let guideURL = '../@pub_guide/guide.html'; //가이드 URL
		let layerPopGuideURL = '../@pub_guide/guide_sample/layerPop_worklist.html'; //레이어팝업 가이드 URL
		let layerPopPath_server = '../../' + fileDir + '/'; // 서버에서 레이어팝업 호출시 경로
		let layerPopPath_local = '../../WEB-INF/' + fileDir + '/'; //로컬에서 레이어팝업 호출시 경로
		let winScroll = 0; //현재 스크롤
		let isLocalStorage = $('html').hasClass('localstorage') ? true : false;
		//네비게이션 로케이션
		let navMap = {
			overNum: -1,
			overNum2: -1,
		...
		let Data = (function () {
			let var_init = function () {
				...
				//th.depth 클래스 수로 저장
				let $th = rMap.tab_content.eq(0).find('thead > tr > th');
				let depSize = 1;
				$th.each(function () {
				...
				//Object Setting : Onum
				for (let i = 1; i <= setting.depSize; i++) {
			...
			let data_init = function () {
				let tmp = {};
				let depNum = setting.depSize;

				//TR 셋팅
				rMap.tab_content.each(function (i) {
					//변수셋팅
					for (let j = 1; j <= depNum; j++) {
						if (j == 1) {
					...
					let ARindex = (dataMap.Odata.ARindex[i] = []);
					let ARcnt = (dataMap.Odata.ARcnt[i] = []);
					let ARtxt = (dataMap.Odata.ARtxt[i] = []);
					let Olist = (dataMap.Odata.List[i] = []);
					let headers = $(this).find('thead > tr > th');
					let cur = {}; //cur.txt1
					//tBody > tr
					$(this)
						.find('tbody > tr')
						.each(function (ii, row) {
							...
							if (cur.txt2 != '' && cur.txt2 != tmp.txt2) {
								...
								for (let jjj = 3; jjj <= depNum; jjj++) {
									if (cur['txt' + jjj] != '') {
										tmp['txt' + jjj] = cur['txt' + jjj];
										tmp['index' + jjj]++;

										for (let k = jjj; k <= depNum; k++) {
											tmp['cnt' + k] = -1;
										}
									}
								}
								...
								if (cur.txt2 != '' && cur.txt2 == tmp.txt2) {
									...
									$(this).find('> td.depth2').addClass('repeat');
								}
							}
							...
							for (let m = 3; m <= depNum; m++) {
								if (cur['txt' + m] != '' && cur['txt' + m] != tmp['txt' + m]) {
									...
									tmp['cnt' + m]++;
									if (tmp['cnt' + m] != 0 && cur['txt' + m] == tmp['txt' + m]) {
										//텍스트 같을때 삭제
										// $(this)
										// 	.find('> td.depth' + m)
										// 	.text('');
					...
					if (setting.debug_mode) {
						$(this)
							.find('> h3')
							.html($(this).find('> h3').text() + ' : [cnt1 : ' + tmp.cnt1 + ']');
					}
				});
			};

			let transData = function () {
				// console.log('\n\n[transData]================');
		 ...
		 */
		let Layout = (function () {
			let appendLayout = function (_callback) {
				let curIndex = 0;
				let tmpIndex = 0;
				let curIndex_sub = 0;
				//tabContent : 번호 자동 추가
        ...
        */
		...
		let Nav = (function () {
			let intervalID;
			let $tabCon = rMap.tab_content;
			let is_fixed = false; //2Depths 고정여부 //contentr와 nav 상태분리
			let is_2Depth_view = false;
			let _show = function (_target) {
				if (!is_2Depth_view) return;
				let $depth1 = rMap.tab_navList;
				let $depth1_on = $depth1.eq(navMap.overNum);
				let $depth2 = $depth1_on.find('.subNav');
				let $depth2_on = $depth2.find('> ul > li').eq(navMap.overNum2);
				...
				rMap.tab_navList.each(function (i) {
					...
					$('.subNav li a', $depth1).on({
						...
						click: function () {
							...
							let curIndex = Number($(this).data('index'));
							let $focusTarget = rMap.tab_content.eq(navMap.activeNum).find('tbody > tr');
							FNfocusOut(); //기존 focus 삭제

							//Header 영역 Foldeing 여부에 따라 높이 설정 (??다시생각);
							let goTop = $focusTarget
								.eq(curIndex - 1)
								.addClass('focus')
								.offset().top;
        ...
        $('.tab_nav > ul').css('width', navW);*/
				...
				rMap.tab_navList.each(function (i) {
					...
					let $subNav = $(this).find('.subNav > ul');

					rMap.tab_content
						.eq(i)
						.find('tbody td.depth2')
						.each(function (ii) {
							let menu2Txt = $(this).text();
							//del 제거후 소팅
		...
		let SideNav = (function () {
			let Onum = dataMap.Onum;

			let init = function () {
				let dep1Len = Number(dataMap.Onum.Len1.length) - 1;
				let btnHtml = '<a href="javascript:void(0);" class="gnbOpenBtn"><i class="fa fa-fw fa-bars"></i><span class="blind">메뉴</span></a>';
				...
				for (let i = 0; i <= dep1Len; i++) {
					let title01 = dataMap.Odata.ARtxt[i][0][0];
					gnbHtml += '				<li>';
			...
			let subinit = function () {
				// console.log('subinit=====: '+Number(dataMap.Odata.ARtxt.length -1));
				let dep1Len = Number(dataMap.Odata.ARtxt.length - 1);
				let gnbHtml = '';
				let depNum = setting.depSize;
				...
				for (let i = 0; i <= dep1Len; i++) {
					let title01 = dataMap.Odata.ARtxt[i][0][0];
					let dep2Len = Number(dataMap.Odata.ARtxt[i].length - 1);

					for (let ii = 1; ii < depNum; ii++) {
					...
					for (let j = 0; j <= dep2Len; j++) {
						let dTxt0 = dataMap.Odata.ARtxt[i][j]; //삭제(.del) 제외(중요)
						if (dTxt0 === '' || dTxt0 === undefined || String(dTxt0) === 'undefined' || dTxt0 == 'NaN' || dTxt0 == null) {
							// console.log('삭제(.del) 제외(중요)')
							// return false;(xxx)
						} else {
							for (let jj = 1; jj < depNum; jj++) {
                ...
                */
								...
								// let _html = '<span class="step step'+k+'\" style="padding-left:'+Number(10*k -10)+'px">└ '+tmpObj['dTxt'+k]+'</span>'
								let _html = '<span class="step" data-cnt=' + k + '" style="padding-left:' + Number(10 * k - 10) + 'px">└ ' + tmpObj['dTxt' + k] + '</span>';
			...
			let subinit_ORI = function () {
				...
				for (let i = 0; i <= dep1Len; i++) {
					let title01 = dataMap.Odata.ARtxt[i][0][0];
					// let dep2Len = Number(dataMap.Onum.Len1[i]-1);
					let dep2Len = Number(dataMap.Odata.ARtxt[i].length - 1);
		 ...
		 * --------------------------------------
		 */
		let TableFolding = (function () {
			// console.log('TableFolding ====')

			//접고 펼치기 아이콘
			let icon_open = 'fa-plus-circle';
			let icon_close = 'fa-minus-circle';
			let foldHtml = '<a href="javascript:;" class="dep_fold_btn fr on"><i class="fa fa-sm fa-fw ' + icon_close + '"</i></a>';
			let watchFolding = [];

			let init = function () {
				let depNum = setting.depSize - 1; // 마지막 폴딩은 삭제
        ...
        */
				...
				rMap.tab_content.each(function (i) {
					...
					for (let d = 2; d <= depNum; d++) {
						...
						//tBody
						let len = dataMap.Onum['Pos' + d][i].length;
						for (let ii = 0; ii < len; ii++) {
							let trPos = Number(dataMap.Onum['Pos' + d][i][ii]) + 1;
							let trNum = Number(dataMap.Onum['Len' + d][i][ii]);
							let $tr = $(this).find('tr').eq(trPos);

							if (trNum > 1) {
								//하위 메뉴가 하나 이상일 경우 버튼 삽입
			...
			let iconToggle = function (_target) {
				let $a = _target;
				let tag = $a.parent()[0].tagName;
				let $curTable = $a.closest('.tab_contents_wrap');
				let curDepNum = $a.parent().attr('class').substr(5); //depth2, depth3...
				...
				//watch
				if (tag == 'TD') {
					let $thBtn = $curTable.find('thead .depth' + curDepNum).find('.dep_fold_btn');
				...
				if (tag == 'TH') {
					let $tdBtn = $curTable.find('tbody .depth' + curDepNum).find('.dep_fold_btn');
					if ($a.hasClass('on')) {
						//console.log('모두다 펼치기 완료 : + > -')
      ...
      */
			function headEventHandler(e) {
				let $a = e;
				let $th = $a.parent();
				let $target = $a.closest('table').find('tbody > tr');
				let curDepNum = $th.attr('class').substr(5); //depth2, depth3 ...
			...
			function bodyEventHandler(e) {
				...
				let $target = $tr.siblings('tr');
				let curDepNum = $td.attr('class').substr(5); //depth2, depth3 ...
				let tmp = {};

				for (let i = 2; i <= curDepNum; i++) {
			...
			let eventHandler = function () {
				rMap.tab_content.find('h3 > .dep_fold_btn').on('click', function () {
					let $a = $(this);
					let $target = $a.parent().parent().find('table > tbody');
		 ...
		 * 	- 무조건 : close 상태
		 */
		let HeaderFolding = (function () {
			let $toggleBtn = Object;
			let fixedFlag = false; //스크롤시 fixed여부
			let headerT_Flag = true; //탑일경우 : true : open,  false : close
			let headerS_Flag = true; //true(접기), false(펼침)

			let appendLayout = function () {
			...
			let eventHandler = function () {
				$toggleBtn.on('click', function () {
					if (rMap.body.hasClass('headerFloating')) {
						// top > 0
		...
		let Quick = (function () {
			let appendLayout = function () {
				let str = '' + '<div class="scroll_btn">' + '<a href="javascipt:void(0);" class="btn_top"><span>↑</span></a>' + '<a href="javascipt:void(0);" class="page_up"><span>∧</span></a>' + '<a href="javascipt:void(0);" class="page_down"><span>∨</span></a>' + '<a href="javascipt:void(0);" class="btn_bottom"><span>↓</span></a>' + '</div>',
			...
			let eventHandler = function () {
				...
				$pgUp.on('click', function () {
					...
					let $diffTop = Math.max(document.documentElement.scrollTop, document.body.scrollTop);
		...
		let ColorBox = (function () {
			...
			let setColorBoxMap = function () {
				...
				cbVar = {
					...
					option: {
						width: '100%',
						height: $(window).height() - bottomGap,
						top: 0,
					},
					autoID: 'autoID',
					autoFlag: false,
				...
				fMap.control_btn.each(function (i) {
					let dataVal = $(this).data('width');
					cbVar.ARresize[i] = dataVal;
				});
			};

			let colorBoxHandler = function () {
				let curSize = cbVar.curSize;
				...
				function addTitle() {
					let prop = $.colorbox.element();
					let curDepNum = prop.closest('.tab_contents_wrap').index();
					let curMenu = $('.tab_nav ul').find('li').eq(curDepNum).text();
					let curTitle = prop.context.title;
					let ARpath = prop.context.pathname.split('/');
					let _path1 = ARpath[ARpath.length - 2];
					let _path2 = ARpath[ARpath.length - 1];
				...
				fMap.footMenu.find('.auto_btn').on('click', function () {
					if (!cbVar.autoFlag) {
						let timeVal = fMap.autoSelect.val();
			...
			let eventHandler = function () {
				rMap.tab_content.find('tr').each(function (i) {
					let target = 'td.num span.number';
					let target_a = $(this).find('.num a');
					// let $pageID = $(this).find('.pid').text();
					let $pageName = $(this).find('.page').text();
					let goURL = $(this).find('.path a').attr('href');
					// let goURL = $(this).find('.path a').attr('href');

					if ($(this).hasClass('del') || $(this).hasClass('disabled') || !$(this).find('.rdate').text() || typeof goURL == 'undefined' || $(this).context.style.display == 'none') {
						target_a.removeClass('iframe cboxElement'); //colorbox object 삭제
					} else {
						//레이어 팝업일경우
						if ($(this).hasClass('layer')) {
							if (url_info.search('file') != -1)
								goURL = layerPopPath_local + goURL; //로컬로 볼경우
              ...
              goURL = $(this).find('.path a').text();
						...
						$(this)
							.find(target)
							.html('<a href=' + goURL + " rel='" + cbVar.className + "' class='" + cbVar.className + "' title='" + $pageName + "'>" + $(this).find(target).text() + '</a>');
					}
				});
		 ...
		 */
		let Filter = (function () {
			let $content = rMap.tab_content.find('tbody');
			let $contentTR = $content.find('tr');
			let $newBtn = rMap.info_section.find('a.new'); //신규
			let $delBtn = rMap.info_section.find('a.del'); //삭제
			let $holdBtn = rMap.info_section.find('a.hold'); //보류
			let $reworkBtn = rMap.info_section.find('a.rework'); //재확인
			let $equalBtn = rMap.info_section.find('a.equal'); //동일
			let $layerBtn = rMap.info_section.find('a.layer'); //레이어팝
			let $popupBtn = rMap.info_section.find('a.popup'); //팝업

			let $appBtn = rMap.info_section.find('a.app'); //APP
			let $webBtn = rMap.info_section.find('a.web'); //WEB

			let $resultBtn = rMap.info_section.find('a.result'); //완료
			let $resultBtn_ex = rMap.info_section.find('a.result_ex'); //미완료
			let $totalBtn = rMap.info_section.find('a.total'); //전체
			let $realBtn = rMap.info_section.find('a.real'); //전체 - 삭제

			let $searchBtn = $('input#id_search');
			let sortType = false; //filter 버튼 클릭시 카운트 재졍렬 여부
			let cnt = 1;
			let delFlag = 0;
			let oldBtn; //클릭한 버튼 저장
			let btnName = 'total'; //전체 버튼 초기셋팅
			let totalCnt = 0; //전체 갯수
			let qs = $searchBtn.quicksearch('.tab_contents_wrap tbody tr'); //Search
			// let childOptionFlag = false // tr.child 옵션일 경우(1-1, 1-2)
			let firstAppendFlag = true; //초기만 td.num html삽입 (total 갯수 Logic)
			let filterOptionObj = {
				delete: true,
				group: false,
			};
			//[filterOption] ===============================//1212
			let option_init = function () {
			...
			function appendNumber() {
				...
				$contentTR.each(function () {
					if (filterOptionObj.group && $(this).hasClass('child')) {
						...
						if (firstAppendFlag) $('td:first-child', $(this)).before('<td class="num" align="center"><span class="number">' + totalCnt + '</span></td>');
						//init
          ...
          $("td:first-child", $(this)).text(index+1);
			...
			function calculator() {
				...
				$contentTR.each(function (index) {
					...
					// let rdateTxtFlag = $(this).find('td.rdate').text() ? true : false; //완료
					let rdateTxtFlag = $(this).find('td.rdate').text() && $(this).find('td.rdate').text() != 'ing' ? true : false; //완료
					// let popTxtFlag = ($(this).find('td.page').text().search('pop') != -1 || $(this).find('td').text().search('popup') != -1) ? true : false;//팝업
					let popTxtFlag = $(this).hasClass('popup'); //팝업
					...
					if (filterOptionObj.delete) {
						...
						if ($(this).hasClass('app')) appCnt++;
						if ($(this).hasClass('web')) webCnt++;
				...
				$appBtn.find('span').text('APP : ' + appCnt);
				$webBtn.find('span').text('WEB : ' + webCnt);
				...
				// let realPercent = Math.round((1-(result_exCnt/(totalCnt - delCnt)))*100);
				realPercent = Math.round((resultCnt / totalCnt) * 100);
				$('.total_rate span').text('완료율 : ' + realPercent + ' % ');
			}

			//Calculator : 각 카테고리별 통계
			function calculator_cate() {
				let str = "<span class='info_calc' style='float: right; padding-right:10px;'> 0/0</span>";
				rMap.tab_title.append(str);
				$('.info_calc').each(function () {
					let $table = $(this).parent().parent().find('table > tbody');
					let _totalNum = $table.find('tr').size() - 1;
					let _childNum = $table.find('tr.child').size();
					let _delNum = $table.find('tr.del').size() - 1;

					let totalNum = _totalNum - _delNum;
					if (filterOptionObj.group) totalNum = totalNum - _childNum;

					let resultNum = 0;
					$table.find('tr td.rdate').each(function (index) {
						let tdStr = String($(this).text());
					...
					let percent = Math.round((resultNum / totalNum) * 100);
			...
			function eventReset($onBtn) {
				...
				$contentTR.each(function (index) {
					// let rdateTxtFlag = $(this).find('td.rdate').text() ? true : false; //완료, 미완료
					let rdateTxtFlag = $(this).find('td.rdate').text() && $(this).find('td.rdate').text() !== 'ing' ? true : false; //완료, 미완료
					...
					if (btnName == 'result' || btnName == 'result_ex') {
						if (rdateTxtFlag) {
							...
							if (btnName == 'result') {
								...
								if (!filterOptionObj.delete && ($(this).hasClass('del') || $(this).hasClass('ing'))) trHide($(this)); //hide //Delete 제외 //ing 제외
							}
						} else {
							//미완료
					...
					//3.Etc BTN
						...
						if ($(this).hasClass(btnName)) {
							trShow($(this));
							let $depth2 = $(this).find('td.depth2');
							let $depth3 = $(this).find('td.depth3');
							let $depth4 = $(this).find('td.depth4');
							let $depth5 = $(this).find('td.depth5');
							$depth2.toggleClass('repeat', prevText2 === $depth2.text());
							$depth3.toggleClass('repeat', prevText3 === $depth3.text());
							$depth4.toggleClass('repeat', prevText4 === $depth4.text());
							$depth5.toggleClass('repeat', prevText5 === $depth5.text());
							prevText2 = $depth2.text();
							prevText3 = $depth3.text();
							prevText4 = $depth4.text();
							prevText5 = $depth5.text();
						}
		...
		let tableOption = (function () {
			...
			let $content = rMap.tab_content.find('tbody');
			let $contentTR = $content.find('tr');
			let $etcCell = $content.find('tr td.etc');
			let $etcDesc = $content.find('tr td.etc .desc');
			let $etcPop = $('.etcPop');
			//비고
			let etcHandler = function () {
				let $focusObj = null;
				...
				$etcCell.each(function () {
					...
					if ($desc.size() > 1) {
						...
						let $btn = $(this).find('.moreBtn');
						let $pop = $('.popArea');
						let $popCon = $('.popArea .popCon');
		...
		let initModule = function () {
			...
			// prevent scrollbar width shift on overflow
			setRoot();

			const setPath = () => {
				$('.tab_contents_wrap tbody tr').each(function () {
					let $tr = $(this);
					let $path = $('td.path', $tr);
					let $a = $('> a', $path);
					let $paths = $('<div class="paths"></div>');
					let $checkbox = $('<input type="checkbox" class="path-state">');
					let $button = $('<button type="button" class="button-mobile"><span class="icon"></span><span class="text">모바일 패널 열기</span></button>');
					$checkbox.prependTo($path);
					$button.appendTo($path);
					$path.wrapInner($paths);
					let $num = $path.siblings('td.num');
					let $rdate = $path.siblings('td.rdate');
					let href = $a.attr('href');

					$tr.toggleClass('popup', $a.text()?.slice(-1) === 'P');
					// prefix = origin + "/websquare/websquare.jsp?w2xPath=";
					// prefix = ($a.hasClass("guide")) ? origin + "/guide" : origin + "/pub";
					// prefix = "http://localhost:3000/view/samples/pub/";
					let prefix = location.origin;
					if ($a.is(':empty')) $a.html(href.match(/[^\/?#]+(?=$|[?#])/));
					// let id = href.substring(1, 5).toLowerCase();
					let id = '';
					// let url = $path.hasClass('sample') ? prefix + href : prefix + id + href;

					if ($path.hasClass('guide')) {
						url = `/guide/${href}.html`;
					} else {
						if (href.toLowerCase().indexOf('launcher') >= 0) {
							url = `/html/${href}`;
						} else {
							let code = href.substring(0, 3);
							url = `/html/${code}/${href}.html`;
						}
					}

					$('.number a', $num).attr('href', url);
					$a.attr('href', url);
					if ($rdate.is(':empty')) {
						$tr.addClass('unreleased');
					} else if ($rdate.text() === 'ing') {
						$tr.addClass('ing');
					} else {
						$tr.addClass('released');
					}
				});
			};
			let $mWindow;
			...
			const resetStates = () => {
				$pStates.each((index) => {
					$pStates.eq(index).prop('checked', false);
				});
			};
			const resetPane = ($element) => {
				$element.find('object').remove();
			};
			const setObject = async (el, url) => {
				el.html(`<object type="text/html" data="${url}" class="mobile-pane"></object>`);
				await sleep(100);
				let $mPane = $('.mobile-pane', el);
				$mPane.attr('data', url);
				// await sleep(1000);
				// $mPane.get(0).contentDocument.children[0].classList.add('mobile-pane-html');
				return true;
			};
			const runPane = ($element) => {
				let $path = $element;
				let $a = $('a', $path);
				let href = $a.attr('href');
				let $pState = $('.path-state', $path);
				resetStates();
				resetPane($mContainer);
				$mState.prop('checked', setObject($mContainer, href)).trigger('change');
				$('.path-state').prop('checked', false);
				$pState.prop('checked', true);
			};

			// 모바일 미리보기
			const setMobilePane = () => {
				$mWindow = $('.mobile-pane-window');
				$mContainer = $('.mobile-pane-container', $mWindow);
				$mState = $('.mobile-pane-state', $mWindow);
				$pStates = $('tr.released .path-state');

				$('.button-mobile').on('click', function (e) {
					runPane($(this).closest('.path'));
				});
				...
				const setPane = (opt) => {
					...
					let $target = false;
					$pStates.each((index) => {
						const idx = opt === 'prev' ? index - 1 : index + 1;
						if ($pStates.eq(index).is(':checked')) {
							$target = $pStates.eq(idx);
						}
					});
					if ($target.length > 0) {
						runPane($target.closest('.path'));
					}
				};

				$(document).keydown((e) => {
					if ($mState.is(':checked')) {
						if (e.keyCode === 37 || e.keyCode === 39) {
							if (e.keyCode === 37) setPane('prev');
							if (e.keyCode === 39) setPane('next');
						}
						if (e.keyCode === 27) {
							$mState.prop('checked', false).trigger('change');
						}
					}
				});

				$('.button-mobile-pane-nav', $mWindow).on('click', function () {
				...
				$mState.on('change', function () {
					...
					$('html').toggleClass('has-mobile-pane', checked);
					$('.path-state').prop('checked', false);
				});
			};

			// Auto Path
			let origin = location.origin.indexOf('127.0.0.1') > -1 ? 'http://localhost:3000' : location.origin;
			setPath();
			setMobilePane();
		};
		return {
