	...
	global.component = function (options) {
		...
		this.pad = function (value, char, length, after) {
			...
			while (result.length < length) {
				result = after ? result + String(char) : String(char) + result;
			}
...
(function (global) {
	...
	global.collapse = (function () {
		...
		function handlerClick(event) {
			...
			$button.hasClass(this.prop('active')) ? hide.call(this, $button, $target) : show.call(this, $button, $target);
	...
	global.select = (function () {
		...
		component.show = function (options) {
			var options = $.extend({ target: null, field: null, on: {} }, options),
				...
				$active = $(this.class('option'), this.class('selector')).filter(
	...
	global.pdf = (function () {
		...
		function slides(options) {
			...
			$.each(
				options.url,
				function (index, url) {
					result += `<div class="swiper-slide ${this.prop('item')}">
			...
			var $html = $(markup),
				...
				width = function () {
					...
					switch (options.direction) {
						case 'top':
						case 'bottom':
							return width - options.padding;
						case 'left':
						case 'right':
							return width - options.padding - options.space - $target.outerWidth();
					}
				}.call(this),
				left = function () {
					...
					switch (options.direction) {
						case 'top':
						case 'bottom':
							return -($target.offset().left - left) - $target.outerWidth() / 2 + options.padding;
						case 'left':
						case 'right':
							return 0;
					}
				...
				translate = function () {
					switch (options.direction) {
						case 'top':
							return `translateY(calc(-100% - ${this.prop('space')}px))`;
						case 'bottom':
							return `translateY(calc(100% + ${this.prop('space')}px))`;
						case 'left':
							return `translateX(calc(-100% - ${this.prop('space')}px))`;
						case 'right':
							return `translateX(calc(100% + ${this.prop('space')}px))`;
					}
				}.call(this);
...
(function (global) {
	...
	global.dropdown = (function () {
		...
		function handlerButton(event) {
			...
			$items.hasClass(this.prop('active')) ? hide.call(this, $items) : show.call(this, $items);
		}
...
(function (global) {
	...
	global.input = (function () {
		...
		function handlerChange(event) {
			...
			$(event.target).val().length ? $clear.addClass(this.prop('active')) : $clear.removeClass(this.prop('active'));
		}
	...
	global.anchor = (function () {
		...
		function handlerTransform(event) {
			var $selector = $(this.class('selector')),
				$button = $(this.class('button')).filter(
			...
			$selector.hasClass(this.prop('scroll')) ? $selector.removeClass(this.prop('scroll')) : $selector.addClass(this.prop('scroll'));
		...
		function handlerClick(event) {
			...
			$(this.prop('scroller'))
				.stop()
				.animate({ scrollTop: scrollTop }, { duration: this.prop('duration') });
			change.call(this, index);
		...
		component.bind = function (options) {
			...
			$(this.prop('container'))
				.on('click', `${this.class('selector')} ${this.class('button')}`, handlerClick.bind(this))
				.click();
